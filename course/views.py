...
def course_delete(request, id):
    ...
    if request.method == 'POST':
        item.delete()
        return redirect('course_list')  # 리스트 화면으로 이동합니다.
    return render(request, 'course_delete.html', {'item': item})
...
def course_list(request):
    ...
    courses = Course.objects.filter(user_id=request.user.pk).order_by('created_at')
    context = {
        'courses': courses,
    }
